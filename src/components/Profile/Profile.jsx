...
function Card({ data }) {
  ...
  return (
    <div className="mx-auto mb-6 h-auto w-full max-w-lg cursor-pointer rounded-lg bg-white p-6 shadow-lg transition-shadow duration-300 hover:shadow-xl dark:bg-textPrimary">
      <div onClick={handleCardClick} className="flex flex-col items-center gap-4 text-center">
        {/* Profile Image */}
        ...
        {/* Name and Location */}
        <h3 className="text-lg font-bold text-textPrimary dark:text-white">{data.name}</h3>
        ...
        {/* Skills */}
        <div className="skills-container mt-4 flex flex-wrap justify-center gap-2" ref={cardRef}>
          {data.skills &&
            data.skills.map((skill, index) => (
              <div className="inline-block rounded-md bg-secondaryColor px-2 py-1 text-xs text-white" key={index}>
                {skill}
              </div>
            ))}
        </div>
      ...
      <div className="mt-4 text-center">
        <div className="text-textSecondary dark:text-white">{data.bio}</div>

...
export default function VoiceSearch({ setVoiceText, isListening, setIsListening }) {
  ...
  return (
    <>
      {isListening ? (
        <div className="fixed left-1/2 top-1/2 z-40 flex  w-72 min-w-64 -translate-x-1/2 -translate-y-1/2 transform flex-col items-center justify-around rounded-lg p-8 bg-[#FFFFFF] dark:bg-[#1E2A47] border-2 border-[#F3F4F6] dark:border-[#141D2F]">
          ...
          <button className="m-2 flex flex-row items-center rounded-md border-2 border-[#00A6FB] bg-[#00A6FB] p-2 text-black transition-colors duration-1000 ease-in-out   hover:bg-white hover:text-[#00A6FB] dark:text-white dark:hover:bg-[#141D2F] px-10" onClick={() => setIsListening(false)}>
            Stop
          </button>
        </div>
      ) : null}

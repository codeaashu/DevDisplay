...
const fetchTrendingPosts = async ({ since = 'day' } = {}) => {
  ...
  try {
    ...
    const $posts = $('.substories .crayons-story')
      .map((i, post) => {
        ...
        const tags = $body
          .find('.crayons-story__indention')
          .find('.crayons-story__tags > a.crayons-tag')
          .get()
          .map((tag) => {
            const $tag = $(tag);
        ...
        const hasReaction = $body
          .find('.crayons-story__bottom > .crayons-story__details > a')
          .find('div.multiple_reactions_aggregate');
        hasReaction.length > 0
          ? (reactions = hasReaction.find('.aggregate_reactions_counter').text().trim())
          : '0 reactions';
...
export { trendingDailyPosts, trendingWeeklyPosts, trendingMonthlyPosts };

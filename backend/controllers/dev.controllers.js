...
const fetchTrendingPosts = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new responseClass.ApiResponse(200, `Fetched trending posts for time period: '${since}'`, posts));
  } catch (error) {
...
const fetchRandomPost = async (req, res) => {
  ...
  try {
    ...
    switch (since) {
      case 'daily':
        posts = await Dev_models.Post_daily.aggregate([{ $sample: { size: 1 } }]);
        break;
      case 'weekly':
        posts = await Dev_models.Post_weekly.aggregate([{ $sample: { size: 1 } }]);
        break;
      case 'monthly':
        posts = await Dev_models.Post_monthly.aggregate([{ $sample: { size: 1 } }]);
        break;
      default:
        posts = await Dev_models.Post_daily.aggregate([{ $sample: { size: 1 } }]);
        break;
    ...
    if (posts.length > 0) {
      ...
      return res
        .status(200)
        .json(new responseClass.ApiResponse(200, `Fetched random post for time period: '${since}'`, posts[0]));
    } else {
      console.log('No documents found.');
      return res
        .status(404)
        .json(new responseClass.ApiResponse(404, `No random posts found for time period: '${since}'`));
    }
  } catch (error) {
    console.error(error);
    return res
      ...
      .json(
        new responseClass.ApiResponse(
          500,
          `Something went wrong while fetching random post for time period: '${since}' --> \n${error.message || error}`,
        ),
      );
  }
};

export { fetchTrendingPosts, fetchRandomPost };

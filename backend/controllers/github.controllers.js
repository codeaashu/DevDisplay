...
const fetchTrendingDevs = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json(new responseClass.ApiResponse(
            200,
            `Fetched trending developers for time period: '${since}'`,
            devs
        ));
...
const fetchTrendingRepos = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json(new responseClass.ApiResponse(
            200,
            `Fetched trending repositories for time period: '${since}'`,
            repos
        ));
...
const fetchRandomDev = async (req, res) => {
    ...
    try {
        ...
        switch (since) {
            case "daily":
                devs = await Dev_models.Dev_daily.aggregate([
                    { $sample: { size: 1 } }
                ])
                break;
            case "weekly":
                devs = await Dev_models.Dev_weekly.aggregate([
                    { $sample: { size: 1 } }
                ])
                break;
            case "monthly":
                devs = await Dev_models.Dev_monthly.aggregate([
                    { $sample: { size: 1 } }
                ])
                break;
            default:
        ...
        if (devs.length > 0) {
            ...
            return res.status(200).json(new responseClass.ApiResponse(200, `Fetched random developer for time period: '${since}'`, devs[0]));
        } else {
            console.log('No documents found.');
            return res.status(404).json(new responseClass.ApiResponse(404, `No random developer found for time period: '${since}'`));
        }
    } catch (error) {
        console.error(error);
        return res.status(500).json(new responseClass.ApiResponse(500, `Something went wrong while fetching random developer for time period: '${since}' --> \n${error.message || error}`));
    }
}
...
const fetchRandomRepo = async (req, res) => {
    ...
    try {
        ...
        switch (since) {
            case "daily":
                repos = await Repo_models.Repo_daily.aggregate([
                    { $sample: { size: 1 } }
                ])
                break;
            case "weekly":
                repos = await Repo_models.Repo_weekly.aggregate([
                    { $sample: { size: 1 } }
                ])
                break;
            case "monthly":
                repos = await Repo_models.Repo_monthly.aggregate([
                    { $sample: { size: 1 } }
                ])
                break;
            default:
        ...
        if (repos.length > 0) {
            ...
            return res.status(200).json(new responseClass.ApiResponse(200, `Fetched random repo for time period: '${since}'`, repos[0]));
        } else {
            console.log('No documents found.');
            return res.status(404).json(new responseClass.ApiResponse(404, `No random repo found for time period: '${since}'`));
        }
    } catch (error) {
        console.error(error);
        return res.status(500).json(new responseClass.ApiResponse(500, `Something went wrong while fetching random repo for time period: '${since}' --> \n${error.message || error}`));
    }
}
